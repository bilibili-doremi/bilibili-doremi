...
function renderNotes(ctx, canvasWidth, canvasHeight, tonic, mode) {
    const { semitones: scale, labels } = buildScale(tonic, mode);

    // Count white keys
    let whiteKeyCount = 0;
    for (let midiNote = FIRST_MIDI; midiNote < FIRST_MIDI + KEY_COUNT; midiNote++) {
...
async function setup() {
  ...
  function checkAndUpdateScale() {
    ...
    if (nextScale && `${nextScale.tonic}_${nextScale.mode}` !== currentKey) {
      ...
      videoElement, nextScale.tonic, nextScale.mode);
    }
  }
